...
def get_data_shearing(path, target_size=(224,224)):
    gen = image.ImageDataGenerator(shear_range=0.1)
    batches = get_batches(path, gen, shuffle=False, batch_size=1, class_mode=None, target_size=target_size)
    return np.concatenate([batches.next() for i in range(batches.nb_sample)])
...
def get_classes(path):
    ...
    return (val_batches.classes, batches.classes, test_batches.classes, onehot(val_batches.classes), onehot(batches.classes), 
        onehot(test_batches.classes), val_batches.filenames, batches.filenames, test_batches.filenames)
